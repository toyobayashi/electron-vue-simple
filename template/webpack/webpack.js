...
const fs = require('fs')
const { main, renderer, dll, mode, manifest } = require('./webpack.config.js')
...
function prod () {
  checkAndBundleDll(() => webpack([main, renderer(manifest)], (err, stats) => {
    if (err) {
      console.log(err)
      return
    }
...
function checkAndBundleDll (callback) {
